...
def test_layer_norm_mean_and_var():
    ...
    assert torch.allclose(
        torch.mean(x_out, dim=-1), torch.zeros(B, T), atol=1e-6
    ) and torch.allclose(
        torch.var(x_out, dim=-1, unbiased=False), torch.ones(B, T), atol=1e-5
    )
...
def test_layer_norm_param_count():
    ...
    assert (
        ...
        and sum(p.numel() for p in LN_no_bias.parameters()) == d_model
    )
