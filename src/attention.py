...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        self, x: Float32[Tensor, "B T_new d_model"], cache: None | KVCache = None
        ...
        )  # each (B, T_new, d_model)
        q, k, v = (
            p.view(B, T_new, self.n_heads, self.head_dim).transpose(-2, -3)
            for p in (q, k, v)
        ...
        start_pos_K = (
            cache.window_start if cache is not None else 0
        )  # abs pos of cache's oldest entry

        q = self.rope(
            q, start_pos=start_pos_Q
        )  # new Q at absolute position total_appended_before
        K_full = self.rope(
            K_full, start_pos=start_pos_K
        )  # recalculate every step to allow for sliding window past T_max

        scores = (
            q @ K_full.transpose(-1, -2) / sqrt(self.head_dim)
        )  # (B, n_heads, T_new, T_total)
        ...
        out = out.transpose(-2, -3).reshape(
            B, T_new, self.d_model
        )  # (B, T_new, d_model)
        output = self.out_proj(out)
