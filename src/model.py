...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = MultiHeadAttention(
            n_heads=n_heads, d_model=d_model, rope_base=rope_base
        )
    ...
    def forward(
        self, x: Float32[Tensor, "B T d_model"], cache: KVCache | None = None
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        for i, block in enumerate(self.blocks):
            ...
            x, _ = block(
                x, layer_cache
            )  # cache is mutated in place, discard the return (_)
        x = self.final_ln(x)
    ...
    @staticmethod
    def apply_top_p_(
        logits: Float32[Tensor, "B V"], p: float
    ) -> Float32[Tensor, "B V"]:
        ...
        sorted_mask[..., 0] = False
        mask = torch.zeros_like(sorted_mask).scatter_(
            dim=-1, index=sorted_ids, src=sorted_mask
        )
    ...
    def stream(
        ...
        yield from self._self_iterator(
            ids=ids,
    ...
    def _self_iterator(
        ...
        try:
            with torch.no_grad():
                cache = (
                    [KVCache(max_size=self.T_max) for _ in range(len(self.blocks))]
                    if use_cache
                    else None
                )
    ...
    def generate(
        ...
        for next_token in self._self_iterator(
            ids,
            max_new_tokens,
            temperature=temperature,
            top_k=top_k,
